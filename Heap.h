...
/********************* Start Microassignment zone ********************/
	...
	void percolateDown(unsigned int hole)
	{
		...
		while(2*hole < _items.size())
		{
			int child1 = 2*hole+1;
			int child2 = child1+1;
			int childholder = child1;
			if(child2 < _items.size() && _items[child2] < _items[child1])
			{
				childholder = child2;
			}

			if(_items[childholder] < _items[hole])
			{
				temp = _items[hole];
				_items[hole] = _items[childholder];
				_items[childholder] = temp;
				hole = childerholder;
			}
			else
			{
	...
	{
    	T temp;
		while(item > 0 && _items[(item-1)/2] > _items[item])
		{
			temp = _items[item];
			_items[item] = _items[(item-1)/2];
			_items[(item-1/)2] = temp;
			item = (item-1)/2;
		}
	}
